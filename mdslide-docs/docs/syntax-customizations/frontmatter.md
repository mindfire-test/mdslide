...
| **`fontSize`**                | `string` | `xs`, `sm`, `md`, `lg`, `xl`, `xxl`                                         | Default typography scale for all slides. Can be overridden per-slide with a `<!-- fontSize: value -->` annotation — see [Slide-Level Annotations](./slide-annotations.md). |
